...
| 8 | `scripts/stage7_violent_typhoon_filtering.py` | the >=106 kt overland ceiling, Section 4c and Fig. 6 |
| 9 | `scripts/stage8_interpolation_check.py` | whether off-synoptic best-track entries are interpolated |
| 10 | `scripts/stage9_model_benchmark.py` | Table 4, all models on one common five-fold split |
| 11 | `scripts/stage10_error_floor_mc.py` | the achievable RMSE floor given best-track uncertainty |
| 12 | `scripts/lf_test2.py` | the land-fraction control of Section 3e |

Figures are regenerated by `scripts/figure2_pareto_native.py` and
`scripts/figure5_stratified_native.py`. Figure 1 is `figures/Tracklines.png`.
Figure 6 panel (a) draws over a web basemap, so a redraw will not be byte-identical
to the archived PNG; `stage7_violent_typhoon_filtering.py` specifies its data layer
in full.

Steps 1 and 2 need the source data described below, as do steps 8 and 9. The rest
need only pandas, numpy and scipy and run in seconds to minutes, except step 10,
...
Table 5(b), `terrain_sweeps.csv` behind the resolution comparison and the
land-fraction control, and the Pareto fronts from all twelve symbolic-regression
seeds.
